...
const howItWorksSteps = [
  {
    ...
    description: 'Pick from a wide range of categories—from plumbing to painting.',
  ...
  return (
    <section className='flex flex-col items-center justify-center px-10 py-16 min-h-svh'>
      <div className='text-center max-w-3xl leading-tight'>
        <h1 className='mt-1 text-[#535252] font-medium text-xl'>How it works</h1>
        <p className='mt-1 text-[#262626] font-bold text-[56px]'>Finding Artisans has Never Been Easier</p>
        <p className='mt-6 text-[#5F5E5E] md:max-w-[70%] mx-auto'>
          Your neighborhood’s best artisans, all in one place. Search, compare offers, and hire with confidence.
        </p>
      </div>

      <div className='grid md:grid-cols-3 gap-3 md:gap-16 max-w-4xl mt-12'>
        {howItWorksSteps.map((step, index) => <Cards key={index} {...step} />)}
      </div>
    </section>
  )
}

const Cards = ({ title, description, Icon }: typeof howItWorksSteps[number]) => {
  return (
    <div className='flex flex-col items-center gap-4 border border-[#E2E8F0] p-4 pt-14 rounded-[12px] text-center'>
      ...
      <div className=''>
        <h2 className='text-[#262626] font-medium text-2xl'>{title}</h2>
        <p className='mt-2 text-[#5F5E5E]'>{description}</p>
      </div>
    </div>
  )
}
